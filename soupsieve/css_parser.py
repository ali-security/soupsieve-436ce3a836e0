...
class CSSParser:
    ...
    def increment_count(self, increment: int = 1) -> None:
        """Increment the selector count and check that it is within the limit."""
    ...
    def parse_combinator(
        ...
        if not has_selector:
            ...
            # Empty slots can never match anything, so there is no need to track them,
            # but they must still be counted so that they cannot be used to exhaust resources.
            if combinator == COMMA_COMBINATOR:
